...
func SetupRoutes(setupRoutes SetupRoutesFunc, roleDMode bool) http.Handler {
	r := chi.NewRouter()
	...
	// Add Role D mode to context for all requests
	r.Use(RoleDModeMiddleware(roleDMode))
