...
func (h *JsonHandlers) ObtainFileHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Call the repository to search for files
	filesRepo := repositories.NewFilesRepository(h.dbPool)
	files, err := filesRepo.Search(r.Context(), parsedUserID, index)
	if err != nil {
		http.Error(w, "Failed to retrieve files", http.StatusInternalServerError)
		return
	}
	
	// Check if there are more files after this page
	// If we got exactly 10 files, there might be more (since our limit is 10)
	hasNext := len(files) == 10
	
	// Check if there are files before this page
	hasPrev := index > 0
	
	// Calculate current page number (1-based)
	page := (index / 10) + 1

	// Prepare response using the models.FilesResponse structure
